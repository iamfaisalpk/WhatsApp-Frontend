    ...
    <div className="overflow-y-auto h-full">
      {filteredChats.map((chat) => {
        ...
        return (
          <div
            key={chat._id}
            // added relative so absolute dropdown is positioned inside this row's stacking context
            className={`flex gap-3 px-4 py-3 hover:bg-[#2a3942] border-b border-[#2a3942]/30 cursor-pointer relative ${selectedChat?._id === chat._id ? "bg-[#2a3942]" : ""}`}
            onClick={() => handleChatSelect(chat)}
          >
            <div className="relative">
            ...
            <div className="text-right flex-shrink-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <div className="relative z-[9999]"> 
                  <button
                    onClick={(e) => handleDropdownToggle(e, chatId)}
                    // make absolutely sure pointer events and cursor are enabled
                    className="p-1 hover:bg-[#2a3942] rounded cursor-pointer pointer-events-auto"
                  ...
                  {isOpen && (
                    <div
                      ref={dropdownRef}
                      // high z-index and pointer-events-auto prevents any invisible overlay from blocking
                      className="absolute right-0 mt-1 w-48 bg-[#233138] rounded-md shadow-lg border border-[#2a3942] z-[10000] py-2 pointer-events-auto"
                      style={{ zIndex: 10000 }}
                    >
                      <button
                        className="w-full px-4 py-2 cursor-pointer text-left hover:bg-[#2a3942] flex items-center gap-3 text-sm"
