          ...
          <div className="overflow-y-auto h-[calc(100%-72px)] scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
            ...
            <div className="flex flex-col items-center text-center p-6">
              <div className="relative mb-4">
                ...
                {isGroupAdmin && (
                  <Edit3 size={16} className="text-[#8696a0] cursor-pointer hover:text-white" />
                )}
                </div>
                ...
                Created by {chat.groupAdmin === currentUser._id ? "You" : chat.groupAdminName || "Admin"}, {new Date(chat.createdAt).toLocaleDateString()}
              </p>
            </div>
            ...
            <div className="px-4 py-3 border-b border-[#2a3942]">
              <div className="flex justify-around">
                {[{ icon: MessageSquare, label: "Media" }, { icon: Phone, label: "Audio" }, { icon: Video, label: "Video" }, { icon: Search, label: "Search" }].map((item, idx) => (
                  <div key={idx} className="flex flex-col items-center space-y-2 cursor-pointer hover:bg-[#2a3942] p-3 rounded">
              ...
              {menuItems.map((item, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    {item.count !== undefined && <span className="text-sm text-[#8696a0]">{item.count}</span>}
                    {item.status && <span className="text-sm text-[#8696a0]">{item.status}</span>}
                    {item.toggle && (
            ...
            <div className="px-4 py-3 border-t border-[#2a3942]">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm text-gray-400">{memberCount} participants</p>
                <Search size={16} className="text-[#8696a0] cursor-pointer hover:text-white" />
              </div>
              ...
              {isGroupAdmin && (
                <div className="flex items-center space-x-3 py-2 hover:bg-[#2a3942] cursor-pointer rounded mb-2">
                  <div className="w-10 h-10 bg-[#00a884] rounded-full flex items-center justify-center">
                    ...
                    <p className="text-sm font-medium text-[#e9edef]">Add participant</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                {chat.members?.map((member) => (
                  <div key={member._id} className="flex items-center space-x-3 py-2 hover:bg-[#2a3942] cursor-pointer rounded">
                    <img src={member.profilePic || "/default-avatar.png"} alt={member.name} className="w-10 h-10 rounded-full object-cover" />
